1+2

seq(11:60)

max(4,5,
    
    3)

#########################

#q()
#help.start()
#help(func_name)
#example(func_name)
#args(func_name)
#help.search(item)
vignette(package="ggplot2")
vignette("ggplot2-specs")


#everything in R is an object
#Basic Data types
#character
#NUMERIC OR decimal
#integer 5L
#Logical TRUE
#complex 1+2i

#how to know which objects is that


class()
typeof()
length()
attributes()
nchar("sairam hi there")

#printing
pi
sqrt(2)
print(pi)

#PRINT KNOWS THE FORMAT where to print
#print can print only one obkect at a time

print("the vlaue of pi",pi,"in two decimals")

print(cat("the value of pi",pi,"with decimals","\n"))
#cat puts a space
#concats make it a one object

#########  setting var

x<-3
y<-5
print(sqrt(x^2+y^3))

#variable held in workspace: we can change variable's data type at will 
x<-"sunday"
print(x)

x<<-5
#global variable
#=assignment: -> can also be used: check the difference 

#Check scope

#########listing variable
ls()
ls.str()
#the str() is called for showing the variable structure 

rm(x) #remove
rm(list= ls())
#Dangerous
ls()

#DATA STRUCTURES
#atomic variable not lists
#types - character,logical,integer,numeric
#empty vector with
a<-vector():a

b<-vector("logical",length=3):b

logical(5)

#or...
vec1=c(1,2,5)#c for combine

vec2=c("sun","moon")
vec3=c(vec1,vec2)

vec4=(vec2as.character("3"));vec4
a<-c(1,0,as.logical(3));a
a<-c(1,0,as.numeric(FALSE));a
a<-c(TRUE,FALSE,as.numeic(2));a

#vectors cannot contain mix data types ...all numbers converted to strings
#data elements can co-exist in a vector only if they have the same mode...

#NA
x<-c("mon",NA,"wed");x


#vectors cannot contain mix data types... all numbers converted to strings
# data elements can co-exist in a vector only if they have the same mode..

# NA
x<-c("mon",NA,"wed");x

#comparing  == != < > <= >=

c(1,2,3,pi)==pi
# Boolean values TRUE FALSE

somenumbers=c(1,2,5,2,3,4);somenumbers
somenumbers[4]=100;somenumbers
somenumbers[4:6]=100;somenumbers

somenumbers[-2]
#excludes that particular value

somenumbers[somenumbers<10]

somenumbers %% 5 ==0
#true for multiples of 5
somenumbers[somenumbers %% 5 ==0]

somenumbers[somenumbers>mean(somenumbers)]
somenumbers[(somenumbers<5)|(somenumbers>95)]

#special values NA, Inf, NaN
length(somenumbers)
somenumbers[7]=NA;somenumbers
somenumbers[17]=NA;somenumbers

is.na(somenumbers)
somenumbers[is.na(somenumbers)]
#existence checking
anyNA(somenumbers)

1/0

0/0


marks<-c(75,85,95)
names(marks)<-c("chem","phy","math") #Interconnection
marks
marks["chem"]

#arithmetic
#create two vectors and try the following
#+ - * / ^ sqrt(vec) log(vec) sin(vec) ....

#operator precedence 

# binary operators %something%
# %% modulo
5%%2
# %/% integer division
5%/%2
# %*% matrix multiplication

# %in% true if left operand occurs in right operand, false otherwise
2 %in% seq(1:10)

# %>% pipe operator





####  sequences and repetitions
1:11
x<-seq(1:11);x
y<-seq(from=2,to=-12,by=-2)
y2<-seq(from=2,to=-12,length.out=5);y2
z=rep(3,times=3);z
10:20
20:10


# basic stats
mean(x)
median(x)
sd(x)
var(x)
cor(x,y)
cov(x,log(x+1))


#pipe (you need library(magrittr) .. 
#for some functions library(dplyr) may be enough)

1:10 %>% mean()
#mean(1:10)

round(pi,3)
pi %>% round(3)
3 %>% round(pi, digits=.)

x<-seq(1:10)
mx=max(x)
mx %>% max(2,.)
mx %>% max(20,.)

paste(10:15,letters[10:15])
10:15 %>% paste(.,letters[.])

sin(cos(90))
f<- . %>% cos() %>% sin()
f(90)


a=10;a=sqrt(a);a

a=10;a<- a %>% sqrt();a

a=10; a%<>%sqrt; a   #%<>% different style of writing pipe operator

# T-junction; tee operator
seq(1:10) %T>% plot(., seq(from = 1,to = 100,by = 10)) %>% mean(.)

#CAT: to print both Numeric and strings

