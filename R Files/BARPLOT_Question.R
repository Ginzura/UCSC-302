pdf_file<-"F:\\2-UG MCS\\barcharts_simple.pdf"
cairo_pdf(bg="grey98",pdf_file,width = 9,height = 6.5)

#Parameters
par(omi=c(0.65,0.25,0.75,0.75),mai=c(0.3,2,0.35,0),mgp=c(3,3,0),family="Lato Light",las=1)

#Import Data and Prepare chart

library(readxl)
ipsos <- read_excel("C:/Users/Student/Downloads/ipsos.xlsx")
sort.ipsos<-ipsos[order(ipsos$Percent),]
attach(sort.ipsos)#Attaching the file to the env


#Create chart

x<-barplot(sort.ipsos$Percent,names.arg = F,horiz = T,border = NA,
           xlim = c(0,100),col = "grey",cex.names = 0.85,
           axes=F)

#Label Chart

for (i in 1:length(sort.ipsos$Country))
{
if (sort.ipsos$Country[i] %in% c("Germany","Brasil"))
  {myFont<-"Lato Black"} else {myFont<-"Lato Light"}
text(-8,x[i],sort.ipsos$Country[i],xpd=T,adj=1,cex=0.85,family=myFont)
text(-3.5,x[i],sort.ipsos$Percent[i],xpd=T,adj=1,cex=0.85,family=myFont)
}
...
arrows(45,-0.5,45,20.5,lwd=1.5,length = 0,xpd=T,col = "yellow")
...
mtext("was said in 2010 in:",3,line=-0.4
      ,adj=0,cex=0.9,outer=T)
mtext("Source: www.ipsos-na.com, Design: Stefan Fitchel, ixtract",1,line=1
      ,adj=1.0,cex=0.65,outer=T,font = 3)

#detach(sort.ipsos)#Detaching after every instance
dev.off()
